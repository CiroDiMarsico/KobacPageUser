...
const create = async (req, res) => {
  ...
  try {
    ...
    const { informacion, carrito, rubro, discount, payments } = req.body
    ...
    for (const item of carrito) {
      if (item.isPromo) {
        // iterar productId → { variantId: cantidad }
        ...
        // producto normal — variants es { variantId: cantidad }
    ...
    if (discount?.code) {
      await discountModel.deactivate(conn, discount.code)  // ← pasar conn
    }
