...
const Modal = ({ title, onClose, children }) => (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70" onClick={onClose}>
        <div
            className="bg-[#0A0A14] border border-white/10 rounded-3xl p-6 w-[90vw] max-w-[480px] flex flex-col gap-5"
...
// ─── Modal producto ───────────────────────────────────────────────────────────
// Sin toggle isActive — el estado se deriva de las variantes
...
const ProductModal = ({ product, categories, onClose, onSaved }) => {
    ...
    const [preview, setPreview] = useState(product?.image     // preview local o URL existente
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (isEdit) {
                await api.put(`/admin/products/${product.id}`, body, authHeaders())
            } else {
                await api.post("/admin/products", body, authHeaders())
            }
...
const VariantModal = ({ variant, productId, onClose, onSaved }) => {
    ...
    const handleSave = async () => {
        ...
        try {
            if (isEdit) {
                await api.put(`/admin/variants/${variant.id}`, { name, isActive }, authHeaders())
            } else {
                await api.post(`/admin/products/${productId}/variants`, { name }, authHeaders())
            }
            onSaved()
        } catch (e) {
            setError(e.response?.data?.error || "Error al guardar")
...
const CategoryModal = ({ rubro, categories, onClose, onSaved }) => {
    ...
    return (
        <Modal title="NUEVA CATEGORÍA" onClose={onClose}>
            <Input label="NOMBRE" value={name} onChange={setName} placeholder="Ej: Energizantes, Sin alcohol..." />
        ...
        <div className="flex items-center gap-4 text-right">
            ...
            <div className="hidden sm:flex flex-col items-end">
                ...
                <span className={`font-['koulen'] text-[14px] ${variant.ganancia > 0 ? "text-green-400" : "text-white/40"}`}>
                    {fmt(variant.ganancia)} <span className="text-[11px]">({fmtPct(variant.porcentaje)})</span>
                </span>
            </div>
            <div className="flex flex-col items-end">
                <span className="font-['koulen'] text-[11px] text-white/30">STOCK</span>
                <span className={`font-['koulen'] text-[15px] ${variant.stock === 0 ? "text-red-400" : "text-white"}`}>
                    {variant.stock}
                </span>
            </div>
        ...
        <div className="border border-white/10 rounded-2xl overflow-hidden">
            ...
            >
                <div className="flex items-center gap-3">
                    ...
                    {product.image && (
                        <img src={product.image} alt={product.name} className="h-9 w-9 object-contain rounded-lg bg-white/5" />
                    )}
                    <div>
            ...
            {editProduct && (
                <ProductModal
                    ...
                    onSaved={() => { setEditProduct(false); onRefresh() }}
                />
            )}
            {editVariant && (
                <VariantModal
                    ...
                    onSaved={() => { setEditVariant(null); onRefresh() }}
                />
            )}
            {newVariant && (
                <VariantModal
                    productId={product.id}
                    onClose={() => setNewVariant(false)}
                    onSaved={() => { setNewVariant(false); onRefresh() }}
                />
            )}
        </div>
    )
        ...
        <div className="p-6 flex flex-col gap-6">
            ...
            {showNewCategory && (
                <CategoryModal
                    ...
                    onSaved={() => { setShowNewCategory(false); fetchData() }}
                />
            )}
        </div>
    )
