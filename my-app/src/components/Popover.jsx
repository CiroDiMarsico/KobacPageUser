...
const Popover = ({ product, onClose, onAgregar, quantities, setQuantities }) => {
    ...
    <div className="fixed inset-0 z-60 flex items-center justify-center bg-black/70" onClick={onClose}>
      <div className="relative bg-[#11111F] rounded-3xl p-6 w-[85vw] h-[80vh] flex flex-col items-center gap-4 font-['prompt']" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex flex-col items-center gap-1 z-10 my-4">
          <h1 className="font-bold text-[35px] leading-none">{product.name}</h1>
                ...
                {variant.stock > 0 &&
                  <div className="flex items-center gap-4">
                    <button
                      ...
                      className="bg-[#C32CFF] rounded-full w-[45px] h-[45px] text-4xl flex items-center justify-center">+</button>
                    <h1 className="font-[koulen] text-[22px]">{quantities[String(variant.id)] ?? 0}</h1>
                    <button
